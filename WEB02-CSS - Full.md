...
### Inline stílusok
A HTML \<head> tagjében megadhatunk stílusokat. Ezek adják majd az oldal designját. \<style> tegek közt direkt megadhatjuk a kulcs-érték párokat. Ne szeretjük, mert nehezen karbantartható, a HTML a tartalomért felelős, nem a designért!
- Szervezzük ki a t0lusokat külön fájlba, és a head-hez adjuk őket hozzá:
...
###### CSS reset
A böngészőknek van egy alapérelmezett stíluslapjuk. Vagyis **Egy elem, amit stílusozni akarunk, sosem "üres"**, mindig minden CSS tulajdonság be van rajta állítva. A spec nem definiálja, mi az alapértelmezett stíluslap tartalma.
...
- [n] `height:`
	...
	- **A % a szülő magasságának %-ában értendő**, ne mindig adja a várt eredményt!
		- A dokumentumfán felfelé **minden elemnek meg kell adni a magasságot, hogy jól működhessen!**
#### Inline elemek
- <mark style="background: #FFF3A3A6;">Egy sorom belül követik egymást</mark>
...
- [n] Egyes CSS property-k kiszakítják az inline elemeket a dokumentumfolyamból, *blokkszerűvé* alakítva őket. Beáll0thatóvá válik így rajtuk a `width, height` érték.

#### Inline-block elemek
Inline:
- [p] Az elemek szépen egyméás mellé kerülnek, és regálnak a vertical align-ra
Blokk:
- [p] Játszhatunk a margókkal, padfinggel, méretekkel
**Nem lehetne mindkettőből egyött egy kicsi?**
...
- [!] Ha vízszintes mergók találkoznak, akkot azokösszeolvadnak és <mark style="background: #FFF3A3A6;">csak a nagyobb lesz látható! </mark> -> **margin collapse**
...
Nem egyértelmű, hogy minek a mérete a `width x height`. Ha belszámoljuk a bordert is és a paddinget is, akkor túlmehetünk a 100% szélességnél! Ilyenkor egymás alá töri őket a layout motor. A megoldás css3-ban a `calc()`:
`width: calc(50%-2px)`;
...
> - Ha egy propery: érték pár támogatott, érvényre jutnak a szabályok, ha nem támogatott, akkor nem jutnak érvényre.
> - Ha egy böngésző egyáltalán nem ismeri a @supports blokkot, afeltételtől függetlenül figyelmen kívül hagyja a tartalmát.
> - Fontos figyelni, hogy a @supports-os szabály legyen később (erősebb).

**vendor prefixek:** a böngészőgyártók megegyeztek abban, hogy m*inden új propertyt prefixszel látnak el, amíg nem véglegesedik a specifikációjuk*, hogy elkerüljék a névütközést a később "szabványra emelkedő" verzióval.
...
### Súlyozás
1. Fontosság
	- Fontos, amit `!important`, minden más nem fontos.
...
3. **Specifikusság**
	- a selector szintjén dől el, a nagyobb „specifikusságú” selector az erősebb.
...
### Dokumentum folyam
A HTML alapvetően egy dokumentumformátum. Ezért a HTML-ben szereplő tartalom dokumentumszerűen próbál viselkedni, balról jobbra, fentről le folyó szövegként. Ez a **normál dokumentumfolyam**. 
...
> - [b] a **none** is a diplayen állítható be.
...
- [!] A `top, right, bottom, left, z-index` tulajdonságok hatástalanok
...
- [!] Az elem új z-rétegre kerül, az eredeti folyambeli elemek fölé. Az új réteghez új koordinátarendszer is dukál, <mark style="background: #FFF3A3A6;">a gyerekelemek abszolút pozícionálása ehhez az elemhez képest lesz abszolút. </mark>
...
#### `position: fixed`
**~ absolute, de itt az elem akkor sem mozog az oldallal, ha elpörgetik**
A top, leftstb. koordinátarendszere: a legközelebbi szülő, amin a transformbármire be van állítva, ha nincs ilyen, akkor a böngészőablak belső széle.
...
#### Normál folyam megörtése float-tal
